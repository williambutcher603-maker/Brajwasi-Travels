...
// ── SESSION ────────────────────────────────────────────────────
app.use(session({
  secret: process.env.SESSION_SECRET || 'brajwasi_2024_secret_xyz',
  resave: false,
  saveUninitialized: false,
  cookie: {
    secure: false,          // set true in prod with HTTPS
    httpOnly: true,
    sameSite: 'lax',        // allows cookie with same-origin fetch
    maxAge: 7 * 24 * 60 * 60 * 1000
  }
}));

// ── File storage ───────────────────────────────────────────────
['public/uploads','public/uploads/docs'].forEach(d => {
  if (!fs.existsSync(d)) fs.mkdirSync(d, { recursive: true });
});
const imgStorage = multer.diskStorage({
  destination: (_, f, cb) => cb(null, 'public/uploads/'),
  filename: (_, f, cb) => cb(null, Date.now() + '-' + f.fieldname + path.extname(f.originalname))
});
const docStorage = multer.diskStorage({
  destination: (_, f, cb) => cb(null, 'public/uploads/docs/'),
  filename: (_, f, cb) => cb(null, Date.now() + '-' + f.fieldname + path.extname(f.originalname))
});
...
const pushCustomer = async (phone, t, b, d = {}) => {
  if (!phone) return;
  sendPushTo(await PushSubscription.find({ role: 'customer', phone: phone.trim() }), t, b, d);
};
...
const sendEmail = async (to, subject, html) => {
  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
    return console.log('⚠  EMAIL_USER/PASS not set in env');
  }
  try {
    const info = await createTransporter().sendMail({
      from: `"Brajwasi Tour & Travels" <${process.env.EMAIL_USER}>`, to, subject, html
    });
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  <url><loc>${base}/driver</loc><changefreq>monthly</changefreq><priority>0.5</priority></url>
</urlset>`);
});
...
app.get('/google*.html', (req, res) => {
  ...
  res.type('text/html').send(`<!DOCTYPE html><html><head><meta name="google-site-verification" content="${file.replace('google','').replace('.html','')}"/></head><body>${file}</body></html>`);
});
...
app.post('/api/auth/send-otp', async (req, res) => {
  ...
  await sendEmail(email, `Your Brajwasi Travels Login OTP: ${otp}`,
    `<div style="font-family:Arial;max-width:480px;margin:0 auto;padding:24px;border:1px solid #e8ddd0;border-radius:12px">
      <h2 style="color:#B8780A;margin-bottom:16px">🔐 Your Login OTP</h2>
      <p style="color:#555">Hello${name ? ' ' + name : ''},</p>
      <p style="color:#555;margin:12px 0">Your one-time password to login to Brajwasi Travels is:</p>
      <div style="font-size:38px;font-weight:800;color:#B8780A;letter-spacing:10px;text-align:center;padding:24px;background:#FFF8EC;border-radius:12px;margin:20px 0;border:2px dashed #B8780A">${otp}</div>
      ...
      <p style="color:#bbb;font-size:12px">Brajwasi Tour & Travels · Agra · 9411061000</p>
    </div>`);
...
app.post('/subscribe', async (req, res) => {
  ...
  await PushSubscription.findOneAndUpdate(
    { endpoint },
    { endpoint, keys, role: role || 'customer', phone: phone || '' },
...
app.post('/api/book', customerAuth, async (req, res) => {
  try {
    const { carId, customerName, customerPhone, pickupLocation, dropLocation,
            journeyDate, journeyTime, estimatedKm, notes } = req.body;
    const customerEmail = req.session.customer.email;

    if (!estimatedKm || +estimatedKm < 1)
      return res.status(400).json({ error: 'Estimated KM is required (minimum 1 km)' });
    ...
    await pushAdmin('🚗 New Booking!', `${customerName} · ${car.model} · ${pickupLocation}→${dropLocation}`,
      { url: '/admin/bookings', bookingId });
    await pushCustomer(customerPhone, '🎉 Booking Received!',
      `Hi ${customerName}! Booking ${bookingId} received. We will confirm shortly.`,
      { url: '/track?bookingId=' + bookingId });

    const emailTo = process.env.EMAIL_TO || process.env.EMAIL_USER;
    await sendEmail(emailTo, `🚗 New Booking ${bookingId} – ${customerName}`,
      `<div style="font-family:Arial;max-width:580px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e8ddd0">
        <div style="background:linear-gradient(135deg,#B8780A,#8A5A06);padding:20px 24px">
          <h2 style="color:#fff;margin:0">🚗 New Booking – ${bookingId}</h2>
        </div>
        ...
        <div style="background:#B8780A;padding:12px 24px;text-align:center">
          <a href="${process.env.BASE_URL||'https://brajwasi-travels.onrender.com'}/admin/bookings" style="color:#fff;text-decoration:none;font-weight:700">View in Admin Panel →</a>
        </div>
      </div>`);
...
app.get('/api/payment/:bookingId', async (req, res) => {
  ...
  if (!upiId) return res.status(500).json({ error: 'UPI_ID not configured in environment' });
  ...
  res.json({
    upiId, bookingId: booking.bookingId,
    upiUrl: `upi://pay?${base}`,
...
app.get('/api/track', async (req, res) => {
  ...
  res.json({
    bookingId: b.bookingId, customerName: b.customerName, carName: b.carName,
    pickupLocation: b.pickupLocation, dropLocation: b.dropLocation,
    journeyDate: b.journeyDate, journeyTime: b.journeyTime,
    status: b.status, advancePaid: b.advancePaid, advanceConfirmedByAdmin: b.advanceConfirmedByAdmin,
    totalPrice: b.totalPrice, finalFare: b.finalFare, actualKm: b.actualKm,
    assignedPartnerName: b.assignedPartnerName,
    driverLat: b.driverLat, driverLng: b.driverLng, driverLastSeen: b.driverLastSeen
  });
});
...
app.post('/api/driver/location', async (req, res) => {
  ...
  if (!bookingId || !lat || !lng || !secret)
    return res.status(400).json({ error: 'bookingId, lat, lng and secret are required' });
  const globalSecret = process.env.DRIVER_SECRET || 'brajwasi_driver';
  const partner = await CarPartner.findOne({ driverSecret: secret });
  if (secret !== globalSecret && !partner)
...
app.post('/api/driver/complete', async (req, res) => {
  ...
  if (!bookingId || !actualKm || !secret)
    return res.status(400).json({ error: 'bookingId, actualKm and secret are required' });
  const globalSecret = process.env.DRIVER_SECRET || 'brajwasi_driver';
  const partner = await CarPartner.findOne({ driverSecret: secret });
  if (secret !== globalSecret && !partner)
    return res.status(403).json({ error: 'Invalid secret code' });
  const booking = await Booking.findOne({ bookingId: bookingId.trim().toUpperCase() });
  if (!booking) return res.status(404).json({ error: 'Booking not found' });
  ...
  // Only deduct ₹500 advance if admin has confirmed it
  const advanceDeducted = booking.advanceConfirmedByAdmin ? 500 : 0;
  const balanceDue = Math.max(0, finalFare - advanceDeducted);
  await Booking.findOneAndUpdate(
    { bookingId: booking.bookingId },
    { actualKm: km, finalFare, status: 'completed' }
  );
  await pushAdmin('🏁 Trip Completed',
    `${booking.customerName} · ${booking.bookingId} · ${km}km · ₹${finalFare}`,
    { url: '/admin/bookings' });
...
app.post('/api/driver/reset-secret/send-otp', async (req, res) => {
  ...
  const otp = genOtp();
  await CarPartner.findByIdAndUpdate(partner._id, {
    resetOtp: otp, resetOtpExpiry: new Date(Date.now() + 10 * 60000)
  });
  await sendEmail(email, `Brajwasi Driver Secret Reset OTP: ${otp}`,
    `<div style="font-family:Arial;max-width:420px;padding:24px;border:1px solid #e8ddd0;border-radius:12px">
      <h2 style="color:#B8780A">Reset Your Driver Secret</h2>
      <p>Your OTP: <strong style="font-size:30px;color:#B8780A;letter-spacing:5px">${otp}</strong></p>
...
app.post('/api/driver/reset-secret/verify', async (req, res) => {
  ...
  const partner = await CarPartner.findOne({ email });
  if (!partner || partner.resetOtp !== otp)
    return res.status(400).json({ error: 'Invalid OTP' });
  if (new Date() > partner.resetOtpExpiry)
    return res.status(400).json({ error: 'OTP expired' });
  await CarPartner.findByIdAndUpdate(partner._id, {
    driverSecret: newSecret.trim(), resetOtp: null, resetOtpExpiry: null
  });
  res.json({ success: true });
});
...
app.post('/api/chat/send', async (req, res) => {
  ...
  const msg = await ChatMessage.create({
    ...
    message: message.trim(), fromCustomer: true, read: false
  });
  await pushAdmin(`💬 ${customerName}`, message.substring(0, 100),
    { url: '/admin', isChat: true, sessionId });
  res.json({ success: true, sessionId, _id: msg._id });
});

app.get('/api/chat/:phone/history', async (req, res) => {
  const messages = await ChatMessage.find({ sessionId: phoneHash(req.params.phone) })
...
app.get('/api/chat/:phone/updates', async (req, res) => {
  ...
  const messages = await ChatMessage.find({
    sessionId: phoneHash(req.params.phone), createdAt: { $gt: since }
  }).sort({ createdAt: 1 });
  res.json({ messages });
});
...
app.post('/api/partner/register',
  ...
  async (req, res) => {
    try {
      const { ownerName, phone, email, carName, carModel, carNumber, seats, ac,
              pricePerKm, fixedKm, fixedPrice, extraKmCharge, driverSecret } = req.body;
      if (!ownerName || !phone || !email || !carName || !carModel || !carNumber
          || !pricePerKm || !extraKmCharge || !driverSecret)
        return res.status(400).json({ error: 'All required fields must be filled' });
      if (driverSecret.length < 6)
        return res.status(400).json({ error: 'Driver secret must be at least 6 characters' });
      ...
      const existing = await CarPartner.findOne({
        $or: [{ phone }, { carNumber: carNumber.toUpperCase() }]
      });
      ...
      const partner = await CarPartner.create({
        ownerName: ownerName.trim(), phone: phone.trim(),
        email: email.trim().toLowerCase(),
        carName: carName.trim(), carModel: carModel.trim(),
      ...
      await pushAdmin('🚗 New Partner Registration',
        `${ownerName} · ${carModel} (${carNumber})`, { url: '/admin' });
      await sendEmail(process.env.EMAIL_TO || process.env.EMAIL_USER,
        `New Partner Registration: ${ownerName}`,
        `<div style="font-family:Arial;max-width:480px">
          <h2 style="color:#B8780A">New Partner Registration</h2>
          <p><b>Name:</b> ${ownerName}</p><p><b>Phone:</b> ${phone}</p>
          <p><b>Car:</b> ${carModel} (${carNumber})</p>
          <p>Login to admin panel to review documents and approve.</p>
        </div>`);
...
app.post('/admin/cars/add', adminAuth, uploadImg.single('image'), async (req, res) => {
  const { name, model, pricePerKm, fixedKm, fixedPrice, extraKmCharge,
          tollTax, availableIn, seats, ac, description } = req.body;
  await Car.create({
    name, model,
    image: req.file ? '/uploads/' + req.file.filename : '/images/default-crysta.jpg',
    pricePerKm: +pricePerKm, extraKmCharge: +extraKmCharge,
...
app.post('/admin/cars/:id/edit', adminAuth, uploadImg.single('image'), async (req, res) => {
  const { name, model, pricePerKm, fixedKm, fixedPrice, extraKmCharge,
          tollTax, availableIn, seats, ac, description } = req.body;
  const upd = {
    name, model, pricePerKm: +pricePerKm, extraKmCharge: +extraKmCharge,
...
app.post('/admin/bookings/:id/status', adminAuth, async (req, res) => {
  ...
  if (req.body.status === 'confirmed')
    await pushCustomer(booking.customerPhone, '✅ Booking Confirmed!',
      `Hi ${booking.customerName}! Booking ${booking.bookingId} is confirmed.`,
      { url: '/track?bookingId=' + booking.bookingId });
  else if (req.body.status === 'cancelled')
    await pushCustomer(booking.customerPhone, '❌ Booking Cancelled',
      `Booking ${booking.bookingId} cancelled. Call 9411061000.`,
      { url: '/track?bookingId=' + booking.bookingId });
...
app.post('/admin/bookings/:id/confirm-advance', adminAuth, async (req, res) => {
  const b = await Booking.findByIdAndUpdate(req.params.id,
    { advancePaid: true, advanceConfirmedByAdmin: true }, { new: true });
  await pushCustomer(b.customerPhone, '✅ Advance Confirmed!',
    `₹500 advance for booking ${b.bookingId} confirmed by admin.`);
  res.json({ success: true });
});
...
app.post('/admin/bookings/:id/assign', adminAuth, async (req, res) => {
  ...
  const booking = await Booking.findByIdAndUpdate(req.params.id, {
    assignedPartner: req.body.partnerId,
    assignedPartnerName: `${partner.ownerName} (${partner.carModel} · ${partner.carNumber})`
  }, { new: true });
  ...
  await sendEmail(partner.email, `Booking Assigned – ${booking.bookingId}`,
    `<div style="font-family:Arial;max-width:540px;margin:0 auto;border:1px solid #e8ddd0;border-radius:12px;overflow:hidden">
      <div style="background:linear-gradient(135deg,#B8780A,#8A5A06);padding:18px 22px">
        <h2 style="color:#fff;margin:0">🚗 New Booking Assigned to You</h2>
      </div>
      <table style="width:100%;border-collapse:collapse;font-size:14px">
      ...
      <div style="padding:14px 18px;background:#fff8ec;font-size:13px;color:#7a6a55">
        Use the Driver app to upload actual KM after completing the trip. Your earnings will be finalised.
      </div>
    </div>`);
...
app.post('/admin/chat/:sessionId/reply', adminAuth, async (req, res) => {
  ...
  await pushCustomer(existing.customerPhone, '💬 Brajwasi replied',
    req.body.message.substring(0, 100), { url: '/', isChat: true });
  res.json({ success: true });
});
...
app.get('/admin/chat/:sessionId/updates', adminAuth, async (req, res) => {
  const since = req.query.since ? new Date(req.query.since) : new Date(0);
  const messages = await ChatMessage.find({
    sessionId: req.params.sessionId, createdAt: { $gt: since }
  }).sort({ createdAt: 1 });
  await ChatMessage.updateMany(
    { sessionId: req.params.sessionId, fromCustomer: true, read: false },
    { $set: { read: true } }
...
app.get('/admin/partners', adminAuth, async (req, res) => {
  res.render('admin/partners', { partners: await CarPartner.find().sort({ createdAt: -1 }) });
});
app.post('/admin/partners/:id/approve', adminAuth, async (req, res) => {
  ...
  await sendEmail(p.email, 'Registration Approved – Brajwasi Tour & Travels',
    `<div style="font-family:Arial;max-width:480px">
      <h2 style="color:#1A7A3A">✅ Registration Approved!</h2>
      <p>Hello ${p.ownerName},</p>
      <p>Your car ${p.carModel} (${p.carNumber}) is approved and active on Brajwasi Travels.</p>
      <p>You will receive booking assignments by email. Commission: ${p.commissionPct}%.</p>
      <p style="color:#7a6a55;font-size:13px;margin-top:12px">Your driver secret code: <strong>${p.driverSecret}</strong><br>Use this in the Driver app to share your live location for assigned trips.</p>
    </div>`);
  res.json({ success: true });
});
app.post('/admin/partners/:id/reject', adminAuth, async (req, res) => {
  ...
  await sendEmail(p.email, 'Registration Update – Brajwasi Tour & Travels',
    `<div style="font-family:Arial;max-width:480px">
      ...
      <p>Hello ${p.ownerName}, unfortunately your registration could not be approved at this time. Please call 9411061000 for details.</p>
    </div>`);
  res.json({ success: true });
});
app.delete('/admin/partners/:id/doc', adminAuth, async (req, res) => {
  ...
  if (p[docField]) {
    try { fs.unlinkSync(path.join(__dirname, 'public', p[docField])); } catch(e) {}
  }
